...
DTset = DAVIS(DAVIS_ROOT , size=(opt.crop_size, opt.crop_size))
...
with torch.no_grad():
    for seq, (inputs, masks) in enumerate(DTloader):
        ...
        video_name = "custom_data"

        save_path = os.path.join(opt.result_path, folder_name,video_name)
        ...
        for t in range(num_frames):
            ...
            total_time += end
            if t>pre:
                print('{}th frame of {} is being processed'.format(t-pre,video_name))
        ...
        if opt.save_video:
            ...
            createVideoClip(final_clip, video_path, '%s.mp4'%(video_name), [opt.crop_size, opt.crop_size])
